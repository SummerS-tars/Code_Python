...
def run_demo() -> None:
    print("运行 demo...")
    # dictionary-based fresh sample data for demo
    records = {}
    ...
    print("\n按学号查询 S001:")
    disp.print_records(records, filter_sid="S001")

    print("\n将 S001 的 English 分数修改为 90:")
    ok, msg = core.update_record(records, "S001", "English", 90)
    print(msg)
    disp.print_records(records)

    print("\n删除 S002 的 Math 记录:")
    ok, msg = core.delete_record(records, "S002", "Math")
    print(msg)
    disp.print_records(records)
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                ...
                n = None
            if n is None or not core.valid_score(n):
            ...
            subjects = core.list_student_subjects(records, sid)
            if not subjects:
                disp.display_message_interactive("未找到匹配记录。")
            else:
                disp.display_records_interactive(records, title=f"学号 {sid} 的记录：", filter_sid=sid)
        elif choice == "3":
            disp.clear_screen()
            ...
            if not subjects:
                disp.display_message_interactive("未找到匹配记录。")
                continue
            for idx, (subj, score) in enumerate(subjects):
                print(f"索引 {idx}: 科目={subj} 分数={score}")
            ...
            try:
                ...
                continue
            if sel_i < 0 or sel_i >= len(subjects):
                disp.display_message_interactive("索引超出范围。")
                continue
            ...
            if n is None or not core.valid_score(n):
                ...
                continue
            subj = subjects[sel_i][0]
            ok, msg = core.update_record(records, sid, subj, n)
            disp.display_message_interactive(msg)
        elif choice == "4":
            disp.clear_screen()
            ...
            if sel_i < 0 or sel_i >= len(subjects):
                ...
                continue
            subj = subjects[sel_i][0]
            ok, msg = core.delete_record(records, sid, subj)
            disp.display_message_interactive(msg)
        elif choice == "5":
